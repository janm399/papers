...
Defining implicit \pcode{String}s demonstrates the principle, but it is rather useless because values of type \pcode{String} is so ubiquitous in typical programs. Different types are sometimes more useful; consider a \pcode{javax.sql.Connection} which might be passed implicitly to various data access code to avoid having to do too much typing. The ability to turn values into values that contain identifiers that are not available on the original types are also quite useful. The real power, though, comes from the fact that the implicit resolution does not stop at the first step. The compiler follows all possible paths, using as many conversions as necessary, to get the code to compile. 
