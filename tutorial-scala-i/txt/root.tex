...
The expression writer implemented earlier hides the fact that it is not a pure function. It simply returns \pcode{Unit}, which forces any side-effects it performs to be blocking. If it were not blocking, the caller would have no way of finding out whether the \pcode{write} operation actually completed. 
