...
Writing \pcode{for (x <- List(1, 2, 3, ...)) yield x * 2} might be more natural way to write \pcode{List(1, 2, 3, ...).map(x => x * 2)}; in simple expressions it can be left largely to one's taste. The real power of Scala's for expressions comes from the fact that it is possible to combine, flatten, map, and filter multiple iterables together. 

The rule to be able to decipher and write for expressions in Scala is to remember that:

\begin{itemize}
  \item the first expression inside \pcode{for} has to be \pcode{element <- iterable},
  \item subsequent \pcode{<-} operation means map and flatten; also known as \pcode{flatMap},
  \item subsequent \pcode{=} operation means \pcode{map},
  \item subsequent \pcode{if} statement means \pcode{filter}
\end{itemize}

This is indeed much so that it inspired a meme in \autoref{fig:lesiterables} and code in \autoref{code:for} with examples of different styles of \pcode{for}.
  ...
  def eval(expr: Expr): Either[Error, Int] = {
    case Div(l, r) =>
      for {
        l <- eval(l)
        r <- eval(r) 
        |$\uparrow$ this doesn't quite solve it; the value of |r| can still be |Right(0)|. Look up and use |Either.filterOrElse|.|
      } yield l / r
    ...
  }
