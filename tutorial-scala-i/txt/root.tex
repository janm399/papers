...
Sum types are very useful way to express alternatives that might not have anything in common other than being of the given type. It is a very useful way to prevent building rich class hierarchies. The \pcode{Person} case class defined above

Employee = Engineer | Sales | Marketing | Executive
...
(Random.nextString(Random.nextInt(16)), Random.nextInt(100)) match {
  ...
  case (x, y) if x.length > 5 && y < 10 => |Both first and second elements are accessible.|
