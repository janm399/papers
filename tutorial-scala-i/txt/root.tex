...
In \pcode{src/main/scala}, in the \pcode{com.acme} package, open \pcode{fizzbuzz.scala} containing the \pcode{FizzBuzzMain} and complete the body of the \pcode{fizzBuzz} function.
...
Build an numerical expression evaluator; the evaluator should support binary operators $+, -, \times, /$ and support arbitrary nesting. It should be able to evaluate, for example $4+3(5/(12-7))$. To make things simpler, it is not necessary to include error reporting (division by zero, for example), and it is not necessary to provide a way to turn a \pcode{String} into an expression. The skeleton is in \pcode{.../eval.scala}.
...
Build a logical expression simplifier that reduces the number of logical operations to be performed, and can spot tautologies and contradictions. For example, $(a \land b) \lor (a \land b)$ should simplify to just $a \land b$; $(a \lor \lnot{a})$ should simplify to $\top$ (tautology; always true); $(a \lor \lnot{a}) \land (a \oplus a)$ should simplify to $\bot$ (contradiction; always false). There is no skeleton code, it will be necessary to create a file in the \pcode{com.acme} package.
...
object SimplifierMain extends App {
  ...
  println(se) | $\rvert$ \Printer{ }|"Contradiction"
}
\end{lstlisting}
\end{example}
...
|The data type syntax is somewhat nicer...|
data Expr a = Const a
            | Plus (Expr a) (Expr a)
            | Minus (Expr a) (Expr a)
            ...
...
|The implementation is written as pattern match.|
eval (Minus l r) = minus (eval l) (eval r)                  
\end{lstlisting}
...
The substitution rule in subtype polymorphism says that a subtype can be used wherever its supertype is expected; \pcode{val x: Animal = new Cat} is valid, but \pcode{val x: AngryCat = new Cat} is not. Following the substitution principle and the behaviour of \pcode{Reader[A]}, \pcode{val x: Reader[Animal] = new Reader[Cat] ...} should be valid, but \pcode{val x: Reader[AngryCat] = new Reader[Cat] ...} should not be valid. The type of the \pcode{read} method in \pcode{Reader[Cat]} returns a value of type that \emph{is} or can be \emph{used in place of} \pcode{Cat}; the code inside the \break \pcode{Reader[AngryCat].read} method contains all the machinery to read \pcode{AngryCat} values; an \pcode{AngryCat} value can certainly stand in place of a \pcode{Cat} value. A value of type \pcode{Reader[AngryCat]} can be used in place of \pcode{Reader[Cat]} as well as \pcode{Reader[Animal]}. Applying the same substitution rule reveals that \pcode{val w: Writer[Cat] = new Writer[AngryCat] ...} should not be valid: the code in \pcode{Writer[AngryCat].write(value: AngryCat)} operates on \pcode{AngryCat}s (the \pcode{write} function must be given \emph{at least} \pcode{AngryCat} as its parameter); an instance of \pcode{Writer[AngryCat]} cannot be substituted for a value of type \pcode{Writer[Cat]}, never mind \break \pcode{Writer[Animal]}. 
...
Unfortunately, this definition would make it somewhat difficult to implement the trait: there would be no way to construct instances of \pcode{M[_]}. The implementation of the \pcode{write} function needs to receive a typeclass instance that allows it to construct a value of type \pcode{M[_]}; and the constructed \pcode{M[_]} should be sequenceable. It should lie in the $\mathfrak{MONAD}$ typeclass. Everyone has heard that ``monad is a simply monoid in the category of endofunctors, so that should make everything absolutely clear,'' together with ``monad is a structure with natural transformations \textmu{} and \texteta{} such that the following diagram commutes.'' What a wonderful way to teach the concept, especially when the task at hand is to implement the \pcode{Writer} that does something useful!\footnote{Monads aren't any more complex than \pcode{AbstractBeanFactoryPostProcessor}; if it's possible to understand \emph{that}, then monads are just as understandable.} 

Monad only describes structure and behaviour of a value; it has nothing as such to do with typeclasses. The structure defines the functions that must be available; behaviour defines how the behaviour behaves when composed with other monads. As a result, many values are monads: \pcode{Option} is a monad, \pcode{List} is a monad, etc. This is where the typeclasses come in: rather than forcing \pcode{Option}, \pcode{List}, etc. to implement some \pcode{Monad} trait (and even if there were such an implementation, how would one go about constructing concrete values given just this interface?), it is better to require there to be an instance of the \pcode{Monad} typeclass for \pcode{Option}, \pcode{List}, but also \pcode{Future}, \pcode{IO}. Ooh!--this is quite useful now.

\begin{example}[Exercise: Flexible writers]
And so it has come to this: implement the expression writer to be usable in caller-defined contexts by requiring its higher-kinded type parameter \pcode{M} to lie in the \pcode{Monad} typeclass. Find a method in the \pcode{Monad} typeclass that can pack a plain value into the monad. 
