...
In addition to the neural network's better performance, it significantly cuts the development time: beyond low-pass filtering on the raw sensor readings, the sensor data does not need any processing. The neural network's output are the classes of exercise. (Or, if trained so, classes of set-up movements.)

% This approach was susceptible to unexpected acceleration values (the user shakes his hand to rest, for example, which provides completely distorted maxima). This is a typical beginner ML problem: eyeballing a few recorded exercises one constructs a hypothesis of what would make good features, but fails when more exercises are added. (And this is before attempting to use the extracted features to train a model like random forest or support vector machine.) So one adds more parameters, introduces more signal processing, ad infinitum: the hand-crafted model does not generalise well.
