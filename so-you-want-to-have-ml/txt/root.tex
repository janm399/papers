...
class ForwardPropagator {
  ...
  static func load(data: NSData) throws {
    ...
  }
}
\end{lstlisting}

Using the SIMD functions for the computational steps gave the application good power-efficiency compared a pure CPU-bound implementation, but larger networks brought significant memory impact. With Core ML's availability, the application's code has become even more energy and memory efficient, though at the small expense of pre-compiling the newly downloaded models from the server (viz \autoref{code:simd}).
...
extension ActivationFunction {
  ...
  func apply(inout input: [Float], 
    ...
    let inputPointer = UnsafeMutablePointer(input)
                        .advancedBy(ofs)
...
Finally, \autoref{code:signal-processor} shows that the interface for the signal processor--the \pcode{SignalProcessor}--is just as simple; however, the implementation again makes most of the SIMD processing capabilities in iOS.
...
\section{Server}
Even though the mobile application can handle all the required computation for a single user, a server-side code is still required to be able to improve the models that the mobile code runs... TODO: Architecture. Ingestion, storage, clustering, model fitting. Hyper-parameter optimisation. Model management.
